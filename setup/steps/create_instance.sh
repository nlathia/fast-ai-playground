...
if [ "$instanceId" = "None" ] || [ "$(aws ec2 describe-instances --profile $profileName --output text --query "Reservations[0].Instances[0].State.Name" --instance-ids $instanceId)" = "terminated" ]; then
