...
if [ "$routeTableId" = "None" ]
  ...
  export routeTableAssoc=$(aws ec2 describe-route-tables --profile $profileName --filters "Name=tag-value,Values=$routeTableName" --query "RouteTables[0].Associations[0].RouteTableAssociationId" --output text)
fi

echo "Your route table id is: $routeTableId"
echo "Your route table association id is: $routeTableAssoc"
